# Magnolia → Silktide deep-link integration template

**Silktide support:** Custom (customer-built)  
**Recommended deep-link approach:** Meta tag (page outputs editor URL)

Use this guide to enable Silktide’s **Edit** button to deep-link users from a page in Silktide to the correct editor screen in **Magnolia**.

## How editing works in this CMS
Magnolia typically edits pages within AdminCentral apps (Pages/Stories) using repository paths and/or UUIDs. Public URLs are often rewritten and do not map 1:1 to AdminCentral URLs.

## Recommended approach and why
Typically edited via AdminCentral/Pages app and internal node paths/UUIDs; no universal 1:1 URL transform from public URL. Use meta tag built from Magnolia page node info.
...
Use Magnolia page node path/UUID available at render time (or a server-side mapping layer) to create the correct AdminCentral deep link.

### Example editor URL patterns
- `AdminCentral deep links vary by setup; typically a hash URL into Pages app with a node path/UUID.`
