# Joomla → Silktide deep-link integration template

**Silktide support:** Custom (customer-built)  
**Recommended deep-link approach:** Meta tag (page outputs editor URL)

Use this guide to enable Silktide’s **Edit** button to deep-link users from a page in Silktide to the correct editor screen in **Joomla**.

## How editing works in this CMS
Joomla content is edited in the administrator backend and the edit screen usually expects an article (or menu item) ID. Public URLs are typically alias-based and don’t include that ID.
...
Use the current Article ID / Item ID from the component rendering the page (e.g., com_content article id) and build the admin edit URL.

### Example editor URL patterns
- `https://{SITE}/administrator/index.php?option=com_content&task=article.edit&id={ARTICLE_ID}`
