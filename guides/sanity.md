# Sanity → Silktide deep-link integration template

**Silktide support:** Custom (customer-built)  
**Recommended deep-link approach:** Meta tag (page outputs editor URL)

Use this guide to enable Silktide’s **Edit** button to deep-link users from a page in Silktide to the correct editor screen in **Sanity**.

## How editing works in this CMS
Sanity is headless: public routes are custom. Editor links require the document ID and Studio base URL.

## Recommended approach and why
Headless: emit meta tag with document ID and studio URL (often uses intents).
...
Use the current document `_id` and your Sanity Studio URL to build a direct edit link (often via a desk tool route).

### Example editor URL patterns
- `https://{STUDIO_HOST}/desk/{DOCUMENT_ID} (varies by Studio routing)`
