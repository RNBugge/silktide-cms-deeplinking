# Tridion Sites (SDL Tridion) → Silktide deep-link integration template

**Silktide support:** Custom (customer-built)  
**Recommended deep-link approach:** Meta tag (page outputs editor URL)

Use this guide to enable Silktide’s **Edit** button to deep-link users from a page in Silktide to the correct editor screen in **Tridion Sites (SDL Tridion)**.

## How editing works in this CMS
Tridion Sites edits content in its UI using publication + item IDs (pages/components). Public URLs are typically generated from content and do not include these identifiers.

## Recommended approach and why
Editing is ID-based (items/components/pages) and not derivable from public URLs.
...
Use the publication ID and item ID available in templates/publishing metadata to construct a URL into the Tridion UI.

### Example editor URL patterns
- `Tridion UI deep links vary by version/tenant; typically require publicationId + itemId.`
