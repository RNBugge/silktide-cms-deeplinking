# Contentful → Silktide deep-link integration template

**Silktide support:** Custom (customer-built)  
**Recommended deep-link approach:** Meta tag (page outputs editor URL)

Use this guide to enable Silktide’s **Edit** button to deep-link users from a page in Silktide to the correct editor screen in **Contentful**.

## How editing works in this CMS
Contentful is headless: public URLs are defined by your frontend routing and don’t inherently contain Contentful entry IDs. Editor URLs require entry IDs and space/environment identifiers.

## Recommended approach and why
Headless: frontend can emit meta tag with the entry ID and a preconfigured space/environment.
...
At render time you usually already have the Entry ID (and Space/Environment). Use those to build the editor URL.
