# BigCommerce → Silktide deep-link integration template

**Silktide support:** Custom (customer-built)  
**Recommended deep-link approach:** Meta tag (page outputs editor URL)

Use this guide to enable Silktide’s **Edit** button to deep-link users from a page in Silktide to the correct editor screen in **BigCommerce**.

## How editing works in this CMS
BigCommerce management URLs typically use internal IDs (products, pages). Public URLs are slug-based, so a deterministic transform is not usually possible without fetching/mapping IDs.

## Recommended approach and why
Admin edit screens are ID-based. Use storefront templates to emit meta tag if you can access product/page IDs; otherwise rely on URL transform is not recommended.
...
Use the current product/page ID from your templating context or via a mapping layer, then build a management URL in the control panel for that entity.

### Example editor URL patterns
- `https://{STORE_DOMAIN}/manage/products/{PRODUCT_ID}`
- `https://{STORE_DOMAIN}/manage/content/pages/{PAGE_ID}`
