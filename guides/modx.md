# MODX → Silktide deep-link integration template

**Silktide support:** Custom (customer-built)  
**Recommended deep-link approach:** Meta tag (page outputs editor URL)

Use this guide to enable Silktide’s **Edit** button to deep-link users from a page in Silktide to the correct editor screen in **MODX**.

## How editing works in this CMS
MODX resources are edited in the manager using a resource ID (`id=`). Public URLs are alias-based.

## Recommended approach and why
Manager edit URLs are resource-ID based; public slugs don’t reliably include IDs.
...
Use the MODX resource ID for the current page and your manager URL.

### Example editor URL patterns
- `https://{SITE}/manager/?a=resource/update&id={RESOURCE_ID}`
