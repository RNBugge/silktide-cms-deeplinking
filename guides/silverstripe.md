# Silverstripe CMS → Silktide deep-link integration template

**Silktide support:** Custom (customer-built)  
**Recommended deep-link approach:** Meta tag (page outputs editor URL)

Use this guide to enable Silktide’s **Edit** button to deep-link users from a page in Silktide to the correct editor screen in **Silverstripe CMS**.

## How editing works in this CMS
Silverstripe edits pages in the CMS interface by internal IDs. Public URLs don’t reliably expose IDs.

## Recommended approach and why
Admin edit routes are ID-based (pages/records).
...
Use the page ID (SiteTree ID) available at render time to build the CMS edit URL.

### Example editor URL patterns
- `https://{SITE}/admin/pages/edit/show/{PAGE_ID}`
