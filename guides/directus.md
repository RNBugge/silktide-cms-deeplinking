# Directus → Silktide deep-link integration template

**Silktide support:** Custom (customer-built)  
**Recommended deep-link approach:** Meta tag (page outputs editor URL)

Use this guide to enable Silktide’s **Edit** button to deep-link users from a page in Silktide to the correct editor screen in **Directus**.

## How editing works in this CMS
Directus admin links are collection + item-id based. Public URLs are custom.

## Recommended approach and why
Editor UI is collection + item-ID based; public URLs are app-specific.
...
Use the collection name and item primary key/ID.

### Example editor URL patterns
- `https://{ADMIN_HOST}/admin/content/{COLLECTION}/{ITEM_ID}`
