# Shopify → Silktide deep-link integration template

**Silktide support:** Custom (customer-built)  
**Recommended deep-link approach:** Meta tag (page outputs editor URL)

Use this guide to enable Silktide’s **Edit** button to deep-link users from a page in Silktide to the correct editor screen in **Shopify**.

## How editing works in this CMS
Shopify admin edit URLs are object-ID based (products, pages, blog posts, articles). The public storefront URL is handle-based, so you need the underlying object id to deep link precisely.

## Recommended approach and why
Admin URLs are ID-based; in Liquid you can access object.id for products/pages/blog posts and build the editorUrl meta tag.
...
In Liquid, you can access IDs for the current object (e.g., `product.id`, `page.id`, `article.id`) and also know the store admin base URL for your shop.

### Example editor URL patterns
- `https://admin.shopify.com/store/{STORE_HANDLE}/products/{PRODUCT_ID}`
- `https://admin.shopify.com/store/{STORE_HANDLE}/pages/{PAGE_ID}`
- `https://admin.shopify.com/store/{STORE_HANDLE}/blogs/{BLOG_ID}/articles/{ARTICLE_ID}`
