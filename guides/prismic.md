# Prismic → Silktide deep-link integration template

**Silktide support:** Custom (customer-built)  
**Recommended deep-link approach:** Meta tag (page outputs editor URL)

Use this guide to enable Silktide’s **Edit** button to deep-link users from a page in Silktide to the correct editor screen in **Prismic**.

## How editing works in this CMS
Prismic is headless: public routes are custom. Editor links rely on the Prismic document ID (and repository).

## Recommended approach and why
Headless: emit meta tag with document ID/UID and repository name.
...
Use the Prismic document ID and repository to build a link to the document in Prismic.

### Example editor URL patterns
- `https://{REPO}.prismic.io/documents~b=working&c=published&l=en-us/{DOCUMENT_ID}`
