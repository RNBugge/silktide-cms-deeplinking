# Sitecore → Silktide deep-link integration template

**Silktide support:** Custom (customer-built)  
**Recommended deep-link approach:** URL transform (derive editor URL from public URL)

Use this guide to enable Silktide’s **Edit** button to deep-link users from a page in Silktide to the correct editor screen in **Sitecore**.

## How editing works in this CMS
Many Sitecore sites allow entering Experience Editor for the current page by adding a known query parameter or switching host/path (implementation varies). If your Sitecore setup requires item IDs or complex routing, you’ll need the meta-tag approach.

## Recommended approach and why
Many Sitecore sites support Experience Editor via predictable URL changes (e.g., query params like sc_mode=edit and/or host swap). If your setup requires item IDs, use the meta tag strategy.

### URL transform rule
**Example:** Public: https://www.example.com/page → Editor: https://www.example.com/page?sc_mode=edit
...
If URL transform works: none (use the current public URL). If not: use the current Sitecore item ID / database info from rendering context to construct a deep link.

### Example editor URL patterns
- `URL transform: {PUBLIC_URL}?sc_mode=edit (common pattern; confirm in your environment)`
