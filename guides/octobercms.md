# October CMS → Silktide deep-link integration template

**Silktide support:** Custom (customer-built)  
**Recommended deep-link approach:** Meta tag (page outputs editor URL)

Use this guide to enable Silktide’s **Edit** button to deep-link users from a page in Silktide to the correct editor screen in **October CMS**.

## How editing works in this CMS
OctoberCMS backend edit URLs are plugin/controller based and typically require record IDs. Public URLs are defined by your routing.

## Recommended approach and why
Back-end editing is record-ID based and front-end URLs are developer-defined.
...
Use the record ID and backend path for your content type.

### Example editor URL patterns
- `OctoberCMS backend deep links are project-specific; typically /backend/{author}/{plugin}/{controller}/update/{ID}`
