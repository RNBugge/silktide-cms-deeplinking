# TYPO3 → Silktide deep-link integration template

**Silktide support:** Custom (customer-built)  
**Recommended deep-link approach:** Meta tag (page outputs editor URL)

Use this guide to enable Silktide’s **Edit** button to deep-link users from a page in Silktide to the correct editor screen in **TYPO3**.

## How editing works in this CMS
TYPO3 editing happens in the backend (typically under `/typo3/`) and pages are edited by internal page UIDs from the page tree. Public URLs usually do not expose those UIDs, so Silktide cannot reliably compute the editor link from the public URL alone.

## Recommended approach and why
TYPO3 backend deep links include internal page uid and often a security token/route; public URL alone isn’t enough. Use meta tag built from page uid + backend route.
...
Use the current page UID available at render time (from TYPO3 page context).

### Example editor URL patterns
- `https://{SITE}/typo3/record/edit?edit[pages][{PAGE_UID}]=edit`
