...
Use this guide to enable Silktide’s **Edit** button to deep-link users from a page in Silktide to the correct editor screen in **Strapi**.

## How editing works in this CMS
Strapi is headless; editor links require the admin base URL, collection name, and record ID. Public URLs are application-defined.
...
Use the Strapi record ID and content type (collection) when rendering.

### Example editor URL patterns
- `https://{ADMIN_HOST}/admin/content-manager/collectionType/api::{COLLECTION}.{COLLECTION}/{RECORD_ID}`
