# Hygraph (GraphCMS) → Silktide deep-link integration template

**Silktide support:** Custom (customer-built)  
**Recommended deep-link approach:** Meta tag (page outputs editor URL)

Use this guide to enable Silktide’s **Edit** button to deep-link users from a page in Silktide to the correct editor screen in **Hygraph (GraphCMS)**.

## How editing works in this CMS
Hygraph (GraphCMS) editor links require project and content IDs. Public URLs are custom.

## Recommended approach and why
Editor is model + entry-ID based; public URLs are app-defined.
...
Use project ID and entry ID/model.

### Example editor URL patterns
- `https://app.hygraph.com/{PROJECT_ID}/content/{MODEL}/{ENTRY_ID}`
